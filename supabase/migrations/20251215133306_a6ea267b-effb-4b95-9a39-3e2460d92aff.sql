...
BEGIN
  ...
  LOOP
    ...
    IF NOT notification_exists THEN
      ...
      -- Notificar todos os admins que têm acesso à empresa do funcionário (sem RETURNING)
      INSERT INTO notifications (user_id, type, message, company_id, document_id, employee_id)
      SELECT 
        ur.user_id,
        CASE 
      ...
      -- Registrar no histórico (uma entrada por documento)
      INSERT INTO notification_history (document_id, days_before_expiration)
      VALUES (doc_record.document_id, days_until_expiration);
    END IF;
  END LOOP;
